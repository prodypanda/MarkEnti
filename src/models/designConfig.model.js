...
const designConfigSchema = new mongoose.Schema({
  ...
  layout: { // Adding layout configuration options
    headerPosition: { type: String, enum: ['top', 'left', 'right', 'bottom'], default: 'top' },
  ...
  logo: { // Adding logo configuration options
  ...
  socialMediaLinks: [ // Array of social media links
    {
  ...
  typography: { // Adding typography options
    fonts: {
  ...
  breakpoints: { // For responsive design settings
  ...
  spacing: { // Adding spacing configuration for margins and padding
  ...
  border: { // Border styles configuration
